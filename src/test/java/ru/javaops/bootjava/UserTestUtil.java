...
public class UserTestUtil {
    ...
    public static List<User> users = List.of(user, admin);
    ...
    public static void assertNoIdEqualsList(List<User> actual, List<User> expected) {
        assertThat(actual).usingRecursiveComparison().ignoringFields("id", "password").isEqualTo(expected);
    }
    ...
    public static List<User> asUsersFromHal(MvcResult mvcResult) throws IOException {
        JsonNode embeddedNode = JsonUtil.getObjectMapper()
                .readTree(mvcResult.getResponse().getContentAsString())
                .path("_embedded")
                .path("users");
        if (embeddedNode.isMissingNode()) {
            return new ArrayList<>();
        } else {
            return JsonUtil.readValues(embeddedNode.toString(), User.class);
        }
    }
    ...
    public static ResultMatcher jsonMatcherList(List<User> expected, BiConsumer<List<User>, List<User>> equalsAssertion) {
        return mvcResult -> equalsAssertion.accept(asUsersFromHal(mvcResult), expected);
    }
}
